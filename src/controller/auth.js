  ...
  */
  static postSignup = async (req, res, next) => {
    ...
    try {
      ...
      res.status(500).json({
        message:
          "something went wrong with our system, please come back later.",
      });
    }
    ...
    try {
      ...
      const jwt = jsonwebtoken.sign(
        { email: save.email, id: save._id },
        "66b0c36b16246afa30d35eaea1fdf71c6aec2bdd075a4d226a0ae33897b63e86",
        { expiresIn: "3h" }
      );
      res.status(202).json({
      ...
      console.log(error);
      res.status(500).json({
        message: "something went wGetResetPasswordrong with our system.",
      });
    }
  };

  //sign in function
  static signiIn = async (req, res, next) => {
    const email = req.body.email;
    const password = req.body.password;
    ...
    if (!validationErrror.isEmpty()) {
      res.status(400).json({
        message: "validation faild check your input",
        error: validationErrror.array(),
      });
      return next();
    }
    ...
    try {
      ...
      if (!user[0]) {
        res
          .status(404)
          .json({ message: "No existing user  with This email. " });
      }

      const ValidateTheThePassword = await bcrypt.compare(
        password,
        user[0].password
      );
      ...
      console.log(error);
      res.status(500).json({
        message: "something went wrong with our system please come back later.",
      });
    }
  };
  ...
  static postResetPassword = async (req, res, next) => {
    const password = req.body.password;
    ...
    try {
      ...
      if (!user[0]) {
        return res.status(403).json({
          message:
            "enable to reste the password please make sure that the the link has not yet expired",
        });
      }
